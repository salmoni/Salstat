...
class PolyBars(PolyPoints):
    ...
    _attributes = {'color': 'black',
                   ...
                   'outline': 'black'}
    def _bar(self, dc, xc,yc,xw,yh):
        dc.DrawRectangle(xc,yc,xw,yh)
    def _drawBars(self,dc,coords):
        f=eval('self._bar')
        xw = 50
        yh = 50
        for xc, yc in coords:
...
#       self.font = self._testFont(font)
    ...
    def draw(self, graphics, xaxis = None, yaxis = None, dc = None, xlabels = None):
        if dc == None: dc = wx.wxClientDC(self)
        dc.BeginDrawing()
        dc.Clear()
        self.last_draw = (graphics, xaxis, yaxis,None, xlabels)
        ...
        self._drawAxes(dc, xaxis, yaxis, p1, p2,
                       scale, shift, xticks, yticks, xlabels)
    ...
    def _drawAxes(self, dc, xaxis, yaxis,
                  bb1, bb2, scale, shift, xticks, yticks, xlabels):
        ...
        if yaxis is not None:
            ...
            for x, d in [(bb1[0], -3), (bb2[0], 3)]:
                ...
                self.lb = p2[1] 
                # this is the coord of the bottom line, not sure how to use it
                dc.DrawLine(p1[0],p1[1],p2[0],p2[1])
...
if __name__ == '__main__':
    ...
    class AppFrame(wx.wxFrame):
        ...
        def OnPlotDraw(self, event):
            self.client.draw(_InitObjects(),'automatic','automatic',None,["high","med","low"]);
